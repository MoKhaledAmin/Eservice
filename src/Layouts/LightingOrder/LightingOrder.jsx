    ...
    const onSubmit = (data) => {
        dispatch(SubmitInfo({ ...data, LONGITUDE: localStorage.getItem('longitude'), LATITUDE: localStorage.getItem('latitude') })).then((res) => {
            ...
            if ((res.payload.MESSAGE?.CODE === null || res.payload.MESSAGE?.CODE === "") && res.payload.DATA !== null) {
                Navigate(`/MunicipalityServices`, { replace: true })
            } else {
                        ...
                        <form onSubmit={handleSubmit((data) => onSubmit(data))}>
                            <div className="container">
                                <div className="row">
                                    ...
                                    {/* <div className="formSelectList col-md-3">
                                        <label className="form-label">{t('PlotNumber')}</label>
                                        <input type="number" className="form-control" name='PLAN_ID' />
                                        <small><ErrorMessage errors={errors} name="PLAN_ID" /></small>
                                    </div> */}
                                </div>
                            </div>
